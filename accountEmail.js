...
export function createAccountMailer({env=process.env, fetchImpl=fetch}={}) {
  ...
  return {
    ...
    async sendVerification(email, token) {
      ...
      const payload={from,to:[email],subject:'Verify your Unpaused email',text,html:`<div style="background:#14191a;color:#f4f6ed;padding:36px;font:16px Arial,sans-serif;max-width:520px"><h1 style="font-size:30px">UNPAUSED</h1><h2>Make it official.</h2><p>Verify your email to activate your account and get your permanent friend code.</p><p style="margin:30px 0"><a href="${escaped}" style="background:#f4f6dc;color:#152415;padding:16px 22px;text-decoration:none;font-weight:bold">VERIFY EMAIL</a></p><p>Choose your account password when the page opens.</p><p style="font-size:13px;color:#b4bfbb">This link expires in 30 minutes and works once. Didn’t request this? You can safely ignore it.</p></div>`};
