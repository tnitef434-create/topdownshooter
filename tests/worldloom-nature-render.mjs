...
try {
 ...
 assert.ok(result.sand.dustParticles>0);assert.equal(result.sand.draws,1);assert.ok(result.sand.grains>0);
 assert.equal(result.ao,true,'dust must be checked with the real AO pipeline enabled');
